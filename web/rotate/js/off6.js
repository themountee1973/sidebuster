...
DEMO.run = function() {
	...
	$('#go').click(function(){
		var val = $(this).text();
		if(val == 'GO') {
			$('<div id="mover">RIGHT</div>').appendTo($(document.body)).animate({ crSpline: spline }, 2000);
			$('<div id="mover2">SETTER</div>').appendTo($(document.body)).animate({ crSpline: spline2 }, 2000);
